...
class Newest extends PureComponent {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      sortArray: [
        ...
        this.props.language['On Sale'],
        this.props.language.Featured,
        this.props.language.Cancel,
      ],
      sortArray2: [
        this.props.language.ASC,
  ...
  componentDidMount() {
    ...
    // const SignInValue = AsyncStorage.getItem('status')
  ...
  async apiLogin() {
    ...
    const SignInValue = await AsyncStorage.getItem('status')
      ...
      if (SignInValue !== null) {
        this.setState({membershipValue : SignInValue});
      }
    ...
    // var raw = JSON.stringify({username: 'real@gmail.com', password: '123456'});
    ...
    // fetch('https://weaclimsolutions.com/getuserdetails.php', requestOptions)
  ...
  applyFilters() {
    ...
    this.setState({products: [], isEmpty: false}, () => {
      this.getProducts();
    });
  }
  ...
  getFilters = async id => {
    const data = await getHttp('products/attributes', {
    ...
    if (data.data.length !== 0) {
      data.data.forEach(element => {
        this.getAttributeValues(element);
      });
    }
  };

  getAttributeValues = async value => {
    const f = value;
    const url = 'products/attributes/' + value.id + '/terms';
  ...
  getProducts = async () => {
    var object = {
      ...
      status: 'publish',
      per_page: theme.perPageNumber,
      order: this.state.sortType, // (asc,desc)
    };
    ...
    if (
      ...
      Object.assign(object, {search: this.state.searchString});
    }

    if (this.state.sortOrder !== '' && this.state.sortOrder !== undefined) {
    ...
    if (
      ...
      if (
        this.props.navigation.dangerouslyGetParent().state.params.brand ===
        'brandId'
      ) {
        Object.assign(object, {on_sale: true});
      }
    }
    ...
    if (
      ...
      if (
        ...
        Object.assign(object, {featured: true});
      }
    }
    ...
    if (
      ...
      Object.assign(object, {category: this.state.selectedTab});
    }
    ...
    if (this.state.applyFilter === true) {
      if (
        ...
        Object.assign(object, {
          min_price: this.state.price.lower,
          max_price: this.state.price.upper,
        });
      }
      if (this.state.selectedFilters.length !== 0) {
        Object.assign(object, {
          attribute: '',
          attribute_term: this.state.selectedFilters[0],
        });
      }
    }

    const data = await getHttp('products', object);
  ...
  onValueChange(value) {
    if (value === 'key0') {
      ...
      this.getSortProducts('price : high - low');
    } else if (value === 'key2') {
      this.getSortProducts('price : low - high');
    } else if (value === 'key3') {
      this.getSortProducts('on sale');
    } else if (value === 'key4') {
      this.getSortProducts('featured');
    } else if (value === 'key5') {
  ...
  getSortProducts2(value) {
    ...
    this.setState(
      {products: [], tempApply: false, page: 1, sortType: value},
      () => {
        this.getProducts();
      },
    );
  ...
  getSortProducts(value) {
    ...
    this.setState(
      {products: [], tempApply: false, page: 1, sortOrder: value},
      () => {
        this.getProducts();
      },
    );
  ...
  //= ======================================================================================
  checkAttributeSelected(fValue, att, v) {
    if (fValue) {
      this.state.selectedFilters.push(v.id);
    } else {
                  ...
                  <TouchableOpacity
                    ...
                    <View
                      ...
                      <Text
                        ...
                        {
                          this.props.language[
                            this.state.indexTemp2.toUpperCase()
                          ]
                        }
                      </Text>
                      <View>
...
const styles = StyleSheet.create({
  ...
  sortText: {
    fontSize: appTextStyle.mediumSize,
    paddingBottom: 0,
    paddingLeft: Platform.OS === 'ios' ? 2 : I18nManager.isRTL ? 32 : 7,
    paddingRight: Platform.OS === 'ios' ? 2 : I18nManager.isRTL ? 8 : 13,
    textAlign:
      Platform.OS === 'ios' ? 'left' : !I18nManager.isRTL ? 'left' : 'right',
  },
  modalOverlay: {
    position: 'absolute',
    top: 0,
