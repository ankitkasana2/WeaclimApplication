...
import { getUrl, getHttp, postHttp, postFetchHttp } from '../common/WooComFetch'
...
class RatingScreen extends Component {
  ...
  //   })
  //   console.log('json222', json)
  //   this.state.reviews = []
  //   if (json.data.length !== 0) {
  //     for (const value of json.data) { this.state.reviews.push(value) }
  //   }
  //   if (json.data.length > 0) {
  //     this.setState({
  //       activityIndicatorTemp: false,
  ...
  try {
    ...
    // Safely initialize reviews array
    let reviews = [];

    // WooCommerce default ya custom API dono cases handle karo
    if (json && json.data && Array.isArray(json.data)) {
      reviews = json.data;
    } else if (Array.isArray(json)) {
      // Agar direct array aa raha ho (kuch cases mein hota hai)
      reviews = json;
    }

    // Ab reviews ko normalize kar do taaki app mein same format rahe
    const normalizedReviews = reviews.map(review => {
      // Rating fix
      const rating = review.rating ? parseInt(review.rating, 10) : 0;

      // Reviewer name logic (sabse important)
      ...
      return {
        id: review.id || review.comment_ID,
        date_created: review.date_created || review.date_created_gmt || review.comment_date,
        review: reviewText,
        rating: rating,
    ...
    // State update properly (never mutate state directly)
    this.setState(
      {
      ...
      () => {
        this.calculateAll(); // ratings average wagera calculate
      }
    );
  } catch (error) {
  ...
  // addComment = async () => {
  //   var dat = {}
  //   dat.author_name = this.props.userData.username
  //   dat.author_email = this.props.userData.email
  //   dat.product_id = this.state.id
  //   dat.author_content = this.state.description
  //   dat.rate_star = this.state.rating
  //   dat.user_id = this.props.userData.id

  //   const json = await postFetchHttp(ThemeStyle.url + '/wp-json/api/tc_settings/app_product_review', dat)
  ...
  // }
    ...
    /////nikita add new addcomment on 18-11-25
    addComment = async () => {
    var dat = {
        author_name: this.props.userData?.username ?? "",
        author_email: this.props.userData?.email ?? "",
        product_id: this.state?.id ?? "",
        author_content: this.state?.description ?? "",
        rate_star: this.state?.rating ?? 1,
        user_id: this.props.userData?.id ?? 0
    };
 
    const json = await fetch(ThemeStyle.url + '/wp-json/api/tc_settings/app_product_review', {
        method: 'POST',
        headers: {
    ...
    if (json.status === 'Success') {
        ...
        this.toast.show(json.message || 'Something went wrong');
    }
      this.setState({
      spinnerTemp: false,
      name: '',
                ...
                <View style={styles.firsyHalfRatingView}>
                  ...
                  <Text
                    style={{
                      ...
                      paddingTop: 3
                    }}>{`${this.state.average.toFixed(2)}`}</Text>
                  <Stars
                    disabled
                    default={parseFloat(
                      1
                  ...
                  <View
                    ...
                    <View style={styles.innerRowView}>
                      ...
                      <Text
                        ...
                        {/* {item.item.reviewer + ' ' + item.item.reviewer} */}
                        {item.item.reviewer}
                      </Text>
                    ...
                    {/* <Text
                      ...
                      <Text
                        ...
                        {(item.item.comment || item.item.review || '')
                          .replace(/<[^>]*>/g, '')   // saare HTML tags hata dega
                          .replace(/&nbsp;/g, ' ')
                          .replace(/&amp;/g, '&')
                          .trim()}
                      </Text>
...
const getCurrencyCode = (state) => state.appConfig.currencyCode
...
const getCurrencyCodeFun = createSelector(
  [getCurrencyCode],
  (getCurrencyCode) => {
...
const mapStateToProps = state => ({
  ...
  currencyCode: getCurrencyCodeFun(state),
